  ...
  extends PieRadarChartBase<PieData>
{
  ...
  private void calcAngles()
  {
    ...
    int i = 0;
    int j = 0;
    while (i < ((PieData)mData).getDataSetCount())
    {
      IPieDataSet localIPieDataSet = (IPieDataSet)localList.get(i);
      int k = 0;
      while (k < localIPieDataSet.getEntryCount())
      {
        mDrawAngles[j] = calcAngle(Math.abs(localIPieDataSet.getEntryForIndex(k).getVal()), f);
        if (j == 0)
        {
        ...
        else
        {
          ...
          arrayOfFloat[j] = (arrayOfFloat[(j - 1)] + mDrawAngles[j]);
        }
        j += 1;
        k += 1;
      }
