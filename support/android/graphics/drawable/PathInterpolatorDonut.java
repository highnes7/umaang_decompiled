  ...
  implements Interpolator
{
  ...
  {
    paramResources = TypedArrayUtils.obtainAttributes(paramResources, paramTheme, paramAttributeSet, R.raw.p);
  ...
  private void inflate(TypedArray paramTypedArray, XmlPullParser paramXmlPullParser)
  {
    ...
    {
      paramTypedArray = TypedArrayUtils.getValue(paramTypedArray, paramXmlPullParser, "pathData", 4);
      paramXmlPullParser = PathParser.toString(paramTypedArray);
      if (paramXmlPullParser != null)
      {
        update(paramXmlPullParser);
        return;
      }
      throw new InflateException(f.sufficientlysecure.rootcommands.util.StringBuilder.toString("The path is null, which is created from ", paramTypedArray));
    }
    if (TypedArrayUtils.get(paramXmlPullParser, "controlX1"))
    {
      if (TypedArrayUtils.get(paramXmlPullParser, "controlY1"))
      {
        ...
        boolean bool = TypedArrayUtils.get(paramXmlPullParser, "controlX2");
        if (bool == TypedArrayUtils.get(paramXmlPullParser, "controlY2"))
        {
          if (!bool)
          {
            create(f1, f2);
  ...
  private void update(Path paramPath)
  {
    ...
    if (k > 0)
    {
      ...
      paramPath = f.sufficientlysecure.rootcommands.util.StringBuilder.append("The Path must start at (0,0) and end at (1,1) start: ");
  ...
  public float getInterpolation(float paramFloat)
  {
    ...
    arrayOfFloat = mY;
    f = arrayOfFloat[j];
    return f.sufficientlysecure.rootcommands.util.StringBuilder.append(arrayOfFloat[i], f, paramFloat, f);
  }
}
